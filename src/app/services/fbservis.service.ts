...
import {  CollectionReference, collection, collectionData, deleteDoc, doc, docData, Firestore, query, setDoc, where } from '@angular/fire/firestore';
...
export class FbservisService {
  ...
  KategoriListele() {
    ...
    return this.aktifUye.pipe(
      concatMap((user) => {
        ...
        return collectionData(myQuery, { idField: 'KategoriId' }) as Observable<Kategori[]>;
      })
    );
  }
    ...
    return this.aktifUye.pipe(
      ...
      concatMap((user) =>
        addDoc(ref, {
         ...
         kataciklama:kategori.kataciklama,
          yayin:kategori.yayin,
 ...
 KategoriSil(kategori : Kategori){
  ...
  ArabaListele() {
    ...
    return this.aktifUye.pipe(
      concatMap((user) => {
        ...
        return collectionData(myQuery, { idField: 'arabaId' }) as Observable<Araba[]>;
      })
    );
  }
  ...
  ArabaDuzenle(araba: Araba) {
    var ref = doc(this.fs, "Arabalar/" + araba.arabaId);
